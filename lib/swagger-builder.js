...
export function findOrCreateTag(tags, tagname) {
...
export function buildMethod(path, method, operation, api) {
  ...
  const hasBodyParam = parameters.some(p => p.in === 'body' || p.in === 'formData')
  // Content-Type should only be the first media type, not a comma list
  const contentType = consumes ? consumes.split(',')[0].trim() : undefined
