...
Handlebars.registerHelper('form-data-helper', (object) => {
  ...
  if (object.indexOf('multipart/form-data') !== -1) {
    ret = `  let formData = new FormData()`
  }
  if (object.indexOf('application/x-www-form-urlencoded') !== -1) {
    ret = `  let formData = new URLSearchParams()`
  }
...
Handlebars.registerHelper('compare-helper', (object) => {
  let ret = ''
  if (object.in === 'query') {
    ret = `    params.querys.push(parameters['` + object.name + `'])`
  }
  if (object.in === 'header') {
    ret = `    params.headers.push(parameters['` + object.name + `'])`
  }
  if (object.in === 'body') {
  ...
  if (object.in === 'formData') {
    ret = `    formData.append('` + object.name + `', parameters['` + object.name + `'])`
  }
  return new Handlebars.SafeString(ret)
})

SwaggerParser.validate('../example/swagger/swagger.yaml', (err, api) => {
  if (err) {
    console.error(err)
  } else {
